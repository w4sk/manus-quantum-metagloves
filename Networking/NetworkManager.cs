...
namespace Manus.Networking
{
	...
	public class NetworkManager : MonoBehaviour
	{
		...
		/// </summary>
		[Tooltip("This variable defines which applications it can find and be found by, should be unique per project.")]
		public string appIdentifier = "MyApp";

		/// <summary>
		/// The port on which the NetworkManager creates the Server at.
		/// </summary>
		[Tooltip("The port on which the Server is created.")]
		...
		/// </summary>
		[Tooltip("The player prefab that is spawned when a player joins.")]
		public NetObject playerPrefab;

		/// <summary>
		/// The world position the Players are spawned in as.
		/// </summary>
		[Tooltip("The world position the Players are spawned in as.")]
		public Vector3 playerSpawnPostion = Vector3.zero;

		/// <summary>
		/// The world rotation the Players are spawned in as.
		/// </summary>
		[Tooltip("The world rotation the Players are spawned in as.")]
		...
		private static int CompareGameObjects( GameObject p_A, GameObject p_B )
		{
			if( p_A.name == p_B.name )
				return p_B.GetInstanceID().CompareTo( p_A.GetInstanceID() );
			return p_B.name.CompareTo( p_A.name );
		}
		...
		void InitializeNetObjectsOnSceneLoaded( UnityEngine.SceneManagement.Scene p_Scene )
		{
			...
			//Sort - good nuff?
			System.Array.Sort( t_Objs, new System.Comparison<GameObject>( CompareGameObjects ) );
