...
namespace Manus.Interaction
{
	...
	/// It should not be added via the editor since the linkage between it and any interactors would be broken.
	...
	public class GrabbedObject : MonoBehaviour
	{
		...
		public class Info
		{
			...
			public Quaternion objectToHandRotation;
		}
		...
		private void OnDestroy()
		{
			...
			{
				if(t_Hand.interactor.grabbedObject==this)
				{
					t_Hand.interactor.Release();
				}
			}
		}
		...
		public bool AddInteractingHand(Info p_GrabInfo)
		{
			...
			{
				if (hands[i].interactor == p_GrabInfo.interactor) return false;
			}
		...
		public bool RemoveInteractingHand(HandGrab p_Hand)
		{
			for (int i = 0; i < hands.Count; i++)
			{
				if (hands[i].interactor == p_Hand)
				{
					var t_Info = hands[i];
					hands.RemoveAt(i);
					foreach (IGrabbable t_G in m_Grabbables)
		...
		public bool GrabbedHandPose( InteractionHand p_InteractionHand )
		{
			for( int i = 0; i < hands.Count; i++ )
			{
				if( hands[i].interactor == p_InteractionHand.handGrab )
				{
					foreach( IGrabbable t_G in m_Grabbables )
					{
						t_G.OnGrabbedHandPose( p_InteractionHand, hands[i] );
					}
					return true;
				}
			}
